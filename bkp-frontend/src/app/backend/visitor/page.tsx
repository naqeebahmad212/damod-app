...
export default function Loginadmin() {
  ...
  const fetchRecords = async () => {
    try {
      const res = await usersApi.getAllUserSearchRecord();
      console.log(res.result);

      setUserRecords(res.result);
  ...
  function filterDocumentsByDateRange(
    ...
    const newDocuument = userRecords.filter(
      (document: any) =>
        ...
        <div className="w-3/4 flex flex-col">
          <div className="flex justify-between">
            <div className="flex gap-4 items-center font-Fahkwang">
              ...
              <h3 className="text-white text-3xl font-medium">Visitor</h3>
            </div>
            <div className="">
              <label className="text-white mr-5 text-xl">Select Period</label>
              <select
                className="bg-white  rounded-md h-10 w-48 px-3 focus:outline-none"
                onChange={(e) =>
                  filterDocumentsByDateRange(userRecords, e.target.value)
                }
              >
          ...
          <div className="mt-10">
            <ul className="text-white flex w-full justify-around bg-zinc-900 px-3 py-2 rounded-md mb-1">
              <li className="font-semibold w-2/12 text-center">Name</li>
              <li className="font-semibold w-2/12 text-center">Phone No</li>
              <li className="font-semibold w-2/12 text-center">
                Certificate No
              </li>
              <li className="font-semibold w-1/12 text-center">Type</li>
              <li className="font-semibold w-2/12 text-center ">Shape</li>
              <li className="font-semibold w-1/12 text-center">Price</li>
              <li className="font-semibold w-4/12 text-center">Email</li>
            </ul>
            {filteredRecords?.map((userRecord: any) => {
              return (
                <ul className="text-white flex w-full justify-around bg-zinc-900 px-3 py-2 rounded-md mb-1">
                  <li className="w-2/12 text-center">{userRecord.user.name}</li>
                  <li className="w-2/12 text-center">
                    {userRecord.user.phoneNumber}
                  </li>
                  <li className="w-2/12 text-center">
                    {userRecord.diamond.certificateNo}
                  </li>
                  <li className="w-1/12 text-center">
                    {userRecord.diamond.category}
                  </li>
                  <li className="w-2/12 text-center capitalize">
                    {userRecord.diamond.shape}
                  </li>
                  <li className="w-1/12 text-center">
                    $ {Math.round(userRecord.diamond.totalAmount * 100) / 100}
                  </li>
